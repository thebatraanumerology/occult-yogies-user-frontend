...
const Logo : React.FC = () =>{
  return(
    <svg viewBox="0 0 870.8 618.9" fill="#781b43">
          ...
          <path d="M523.9,222.2" />
          <path d="M341.8,222.2" />
          <path d="M167.9,163.5C167.8,163.1,167.9,164.2,167.9,163.5Z" />
          <path d="M212.2,236C212.6,235.7,212.1,235.9,212.2,236Z" />
          <path d="M697.8,163.5C697.8,164.2,698,163.1,697.8,163.5Z" />
          ...
          <path
            d="M789.1,468.9l-17.2,14.2,9.3,14.4,17.2-14.3-9.2-14.3Z"
            fill="#efba49"
          />
          <path
            d="M845.4,503.9h14.9L856.7,517c4.7-6.5,9.5-13,14.1-19.5l-25.3,6.4Z"
            fill="#efba49"
          />
        </svg>
  )
...
const NavBar = () => {
  ...
  const [name, setName] = useState<string>("Vivek Pathak");

  return (
    <nav className="flex items-center justify-between px-12 py-1 bg-white/30 z-10 shadow-sm shadow-black/10">
        ...
        >
          ...
          {showProfileDropdown && (
            <div className="absolute right-0 top-full w-64 rounded-xl bg-white mt-3 p-4 z-50 shadow-lg shadow-black/20 border border-black/20">
              ...
              {/* Actions */}
              <div className="flex flex-col gap-3 text-sm font-400 ">
                <NavLink
                  to="/profile"
                  className="flex items-center gap-2 hover:text-black/30"
                >
                  <SquarePen size={18} />
                  Edit Profile
                </NavLink>

                <button className="flex items-center gap-2 hover:text-black/30 text-left">
                  <LogOutIcon size={16} />
                  Logout
                </button>
              </div>
